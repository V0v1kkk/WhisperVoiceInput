...
public class PipelineIntegrationTests : AkkaTestBase
{
    ...
    public void Should_Handle_AudioRecording_Failure_And_Stay_In_Recording_State()
    {
        ...
        var orchestrator = ActorOfAsTestFSMRef<MainOrchestratorActor, AppState, StateData>(
            Props.Create(() => new MainOrchestratorActor(
                    ...
                    _observerProbe.Ref))
                .WithDispatcher(CallingThreadDispatcher.Id),
            "test-orchestrator-recording-failure"
        );
        ...
        // Give some time for retries to happen
        TestScheduler.Advance(TimeSpan.FromSeconds(1));

        // AudioRecording should fail and get restarted, but the StopRecordingCommand message is lost
        // So the orchestrator stays in Recording state waiting for AudioRecordedEvent that never comes
        orchestrator.StateName.Should().Be(AppState.Recording, 
            "Should remain in Recording state when StopRecordingCommand fails repeatedly");

        // Verify we can still send new commands
        orchestrator.Tell(new ToggleCommand()); // Try to start recording again
        orchestrator.StateName.Should().Be(AppState.Recording, 
            "Should still be able to handle new commands in Recording state");
    }

    [Test]
    public void Should_Handle_Transcription_Failure_And_Stay_In_Transcribing_State()
    {
        // Arrange - Factory that will make TranscribingActor always fail
        var failingPropsFactory = new MixedActorPropsFactory(
            _recordingDelay, _transcriptionDelay, _postProcessingDelay, _savingDelay, TestScheduler,
            failTranscribing: true);
        ...
        var orchestrator = ActorOfAsTestFSMRef<MainOrchestratorActor, AppState, StateData>(
            Props.Create(() => new MainOrchestratorActor(
                    ...
                    _observerProbe.Ref))
                .WithDispatcher(CallingThreadDispatcher.Id),
            "test-orchestrator-transcription-failure"
        );
        ...
        // TranscribingActor should fail and get restarted, but the TranscribeCommand message is lost
        // So the orchestrator stays in Transcribing state waiting for TranscriptionCompletedEvent that never comes
        orchestrator.StateName.Should().Be(AppState.Transcribing, 
            "Should remain in Transcribing state when TranscribeCommand fails repeatedly");

        // Verify we can still send new commands (though they may be stashed)
        orchestrator.Tell(new UpdateSettingsCommand(TestSettings with { Model = "new-model" }));
        orchestrator.StateName.Should().Be(AppState.Transcribing, 
            "Should still be able to handle new commands in Transcribing state");
    }

    [Test]
    public void Should_Handle_PostProcessor_Failure_And_Stay_In_PostProcessing_State()
    {
        // Arrange - Settings with post-processing enabled, but PostProcessor will fail
        var settingsWithPostProcessing = CreateSettingsWithPostProcessing();
        var failingPropsFactory = new MixedActorPropsFactory(
            _recordingDelay, _transcriptionDelay, _postProcessingDelay, _savingDelay, TestScheduler,
            failPostProcessor: true);
        ...
        var orchestrator = ActorOfAsTestFSMRef<MainOrchestratorActor, AppState, StateData>(
            Props.Create(() => new MainOrchestratorActor(
                    ...
                    _observerProbe.Ref))
                .WithDispatcher(CallingThreadDispatcher.Id),
            "test-orchestrator-postprocessor-failure"
        );

        // Act - Complete pipeline through transcription
        orchestrator.Tell(new ToggleCommand());
        orchestrator.Tell(new ToggleCommand());
        ...
        // PostProcessor should fail and get restarted, but the PostProcessCommand message is lost
        // So the orchestrator stays in PostProcessing state waiting for PostProcessedEvent that never comes
        orchestrator.StateName.Should().Be(AppState.PostProcessing, 
            "Should remain in PostProcessing state when PostProcessCommand fails repeatedly");

        // Verify we can still send new commands (though they may be stashed)
        orchestrator.Tell(new UpdateSettingsCommand(TestSettings with { Model = "new-model" }));
        orchestrator.StateName.Should().Be(AppState.PostProcessing, 
            "Should still be able to handle new commands in PostProcessing state");
    }
    ...
    [Test]
    public void Should_Handle_Multiple_Concurrent_Failures_And_Stay_In_Recording_State()
    {
        // Arrange - Multiple actors will fail
        var failingPropsFactory = new MixedActorPropsFactory(
            _recordingDelay, _transcriptionDelay, _postProcessingDelay, _savingDelay, TestScheduler,
            failAudioRecording: true,
            failTranscribing: true,
        ...
        var orchestrator = ActorOfAsTestFSMRef<MainOrchestratorActor, AppState, StateData>(
            Props.Create(() => new MainOrchestratorActor(
                    ...
                    _observerProbe.Ref))
                .WithDispatcher(CallingThreadDispatcher.Id),
            "test-orchestrator-multiple-failures"
        );

        // Act - Start pipeline with multiple potential failures
        orchestrator.Tell(new ToggleCommand()); // Start recording
        orchestrator.StateName.Should().Be(AppState.Recording);
        ...
        // With multiple failing actors, the AudioRecordingActor fails first on StopRecordingCommand
        // The orchestrator stays in Recording state waiting for AudioRecordedEvent that never comes
        orchestrator.StateName.Should().Be(AppState.Recording, 
            "Should remain in Recording state when multiple actors fail, starting with AudioRecording");

        // Verify we can still send new commands
        orchestrator.Tell(new ToggleCommand()); // Try to send another command
        orchestrator.StateName.Should().Be(AppState.Recording, 
            "Should still be able to handle new commands despite multiple failures");
    }

    [Test]
    public void Should_Handle_Failed_Actor_And_Stay_In_Transcribing_State()
    {
        // Arrange - TranscribingActor that always fails
        var failingPropsFactory = new MixedActorPropsFactory(
            _recordingDelay, _transcriptionDelay, _postProcessingDelay, _savingDelay, TestScheduler,
            failTranscribing: true);
        ...
        var orchestrator = ActorOfAsTestFSMRef<MainOrchestratorActor, AppState, StateData>(
            Props.Create(() => new MainOrchestratorActor(
                    ...
                    _observerProbe.Ref))
                .WithDispatcher(CallingThreadDispatcher.Id),
            "test-orchestrator-stuck-transcribing"
        );

        // Act - Pipeline that will get stuck in transcribing
        orchestrator.Tell(new ToggleCommand()); // Start recording
        orchestrator.Tell(new ToggleCommand()); // Stop recording
        ...
        // Give some time for failure retries
        TestScheduler.Advance(TimeSpan.FromSeconds(1));
            
        // TranscribingActor should fail and get restarted, but message is lost
        // So orchestrator stays in Transcribing state waiting for TranscriptionCompletedEvent that never comes
        orchestrator.StateName.Should().Be(AppState.Transcribing, 
            "Should remain stuck in Transcribing state when TranscribeCommand fails repeatedly");

        // Verify we can still send commands (though they will be stashed)
        orchestrator.Tell(new UpdateSettingsCommand(TestSettings with { Model = "new-model" }));
        orchestrator.StateName.Should().Be(AppState.Transcribing, 
            "Should still accept commands even when stuck in Transcribing state");
        ...
        stateEvents.Should().Contain(evt => evt.State == AppState.Transcribing,
            "Should have received Transcribing state notification");
    }
